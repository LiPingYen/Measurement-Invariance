...
dta_list<-replicate(n=reps,data.frame(gen_dta_v6(nobs = nobs,la1=lambda1_1,la2 = lambda2_4,
                                                phi1 = phi1,phi2 = phi2,
                                                th2 = theta2_4)),simplify = FALSE)
...
rmsea_all_c4_n200<-rmfun_papl(md=md,dta = dta_list)
data.frame(mean=mean(rmsea_all_c4_n200),sd=sd(rmsea_all_c4_n200))
...
cpu.cores <- detectCores()
cl <- makeCluster(cpu.cores,type = "FORK")
rmsea_all_c4_n500<-rmfun_papl(md=md,dta = dta_list)
data.frame(mean=mean(rmsea_all_c4_n500),sd=sd(rmsea_all_c4_n500))
...
phi1<-1
lambda2_4<-matrix(c(0.6,0.77,0.5,0.51,0.69,0.3,0.6,0.77,0.5,0.51,0.69,0.3),nrow = 12)
phi2<-1.3
theta2_4<-diag(c(0.8,1.3,0.4,0.6,0.9,0.2,0.8,1.3,0.4,0.6,0.9,0.2))

dta_list<-replicate(n=reps,data.frame(gen_dta_v12(nobs = nobs,la1=lambda1_1,la2 = lambda2_4,
                                                phi1 = phi1,phi2 = phi2,
                                                th2 = theta2_4)),simplify = FALSE)
...
####false detection####
fal_det_pl(non_var = non_var,inv_con = c(".p3.",".p6.",".p9.",".p12."))
