...
dta_list<-replicate(n=reps,data.frame(gen_dta_v6(nobs = nobs,la1=lambda1_1,la2 = lambda2_2,
                                                phi1 = phi1,phi2 = phi2,
                                                th2 = theta2_2)),simplify = FALSE)
...
rmsea_all_c2_n200<-rmfun_papl(md=md,dta = dta_list)
data.frame(mean=mean(rmsea_all_c2_n200),sd=sd(rmsea_all_c2_n200))
...
cpu.cores <- detectCores()
cl <- makeCluster(cpu.cores,type = "FORK")
rmsea_all_c2_n500<-rmfun_papl(md=md,dta = dta_list)
data.frame(mean=mean(rmsea_all_c2_n500),sd=sd(rmsea_all_c2_n500))
...
phi1<-1
lambda2_2<-matrix(c(0.5,0.64,0.5,0.6,0.8,0.3,0.5,0.64,0.5,0.6,0.8,0.3),nrow = 12)
phi2<-1.3
theta2_2<-diag(c(0.7,1.3,0.4,0.5,0.9,0.2,0.7,1.3,0.4,0.5,0.9,0.2))

dta_list<-replicate(n=reps,data.frame(gen_dta_v12(nobs = nobs,la1=lambda1_1,la2 = lambda2_2,
                                                phi1 = phi1,phi2 = phi2,
                                                th2 = theta2_2)),simplify = FALSE)
