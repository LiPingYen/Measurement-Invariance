...
dta_list<-replicate(n=reps,data.frame(gen_dta_v6(nobs = nobs,la1=lambda1_1,la2 = lambda2_7,
                                                phi1 = phi1,phi2 = phi2,
                                                th2 = theta2_7)),simplify = FALSE)
...
rmsea_all_c7_n200<-rmfun_papl(md=md,dta = dta_list)
data.frame(mean=mean(rmsea_all_c7_n200),sd=sd(rmsea_all_c7_n200))
...
cpu.cores <- detectCores()
cl <- makeCluster(cpu.cores,type = "FORK")
rmsea_all_c7_n500<-rmfun_papl(md=md,dta = dta_list)
data.frame(mean=mean(rmsea_all_c7_n500),sd=sd(rmsea_all_c7_n500))
